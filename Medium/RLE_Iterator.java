...
The iterator supports one function: next(int n), which exhausts the
next n elements (n >= 1) and returns the last element exhausted in this way.
...
/*
    Approach: Index tracker starting at zero and increments by 2. If value at index is
              greater than or equal to n, decrement by n and return the value after index.
              Otherwise jump index by 2.
*/
    // O(n) time for next worst case if each count is 1
    // O(1) space
    // Runtime: 5 ms, faster than 96.71% of Java online submissions
    // Memory Usage: 39.1 MB, less than 96.38% of Java online submissions
    class RLEIterator {
        ...
        public int next(int n) {
            ...
            return (index < A.length && A[index] > 0) ? A[index + 1] : -1;
        }
    }
